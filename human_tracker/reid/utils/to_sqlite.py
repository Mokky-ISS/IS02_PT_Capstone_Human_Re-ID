...
def insert_vector_db(img_id, img, feat_vec):
    try:
        sqliteConnection = sqlite3.connect('reid_db.db')
        cursor = sqliteConnection.cursor()
    
        Query = """
            INSERT INTO vectorkb_table (img_id, img, vector_tensor) 
            VALUES (?,?,?)
            """

        array = (img_id, img, feat_vec)
...
def load_gallery_from_db():
    try:
        ...
        cursor.execute("select img_id, img, vector_tensor from vectorkb_table")
        ...
        cursor.close()
        sqliteConnection.close()
        return img_id, patch_img, gal_feat
